...
public struct TermsAndConditionsView: View {
                ...
                ScrollViewReader { proxy in
                    ScrollView {
                        ...
                        Group {
                            ...
                            .padding(.leading)
                            highlightedText("\nImportant: NexGenSpec software stores inspection records on the Inspector's device and, when iCloud Sync is on (the default), syncs the inspection record, report PDFs, thumbnails, and LiDAR floor plans across the Inspector's own Apple devices through the Inspector's private iCloud account (Apple CloudKit). The records go only to the Inspector's iCloud — NexGenSpec never receives or stores them. Original full-resolution photos, videos, and 3D scan files never sync and exist only on the device that captured them — losing that device loses those originals even with iCloud Sync on, so back them up separately. NexGenSpec does not host, backup, audit, or guarantee retention of these records, and iCloud Sync is not a backup (deletions sync across the Inspector's devices). NexGenSpec cannot recover records if the Inspector deletes them, deletes their account, or loses all of their devices and copies. The Inspector is fully responsible for backing up records to meet this retention practice (e.g. iCloud Backup + per-inspection export to Files app / iCloud Drive).\n\nEmail delivery and retention. Once a PDF report has been delivered to the client by email, retention of the client's copy is governed entirely by the client's chosen email/storage provider — not by the Inspector and not by NexGenSpec. The Inspector should not rely on a sent email as a long-term archive.\n", font: .body)
                            Divider()
                        }
                        .id("section2")
                    }
    ...
    public var fullTermsText: String {
        ...
        Important: NexGenSpec software stores inspection records on the Inspector's device and, when iCloud Sync is on, syncs the inspection record, report PDFs, thumbnails, and LiDAR floor plans across the Inspector's own Apple devices through the Inspector's private iCloud account. Original full-resolution photos, videos, and 3D scan files never sync and exist only on the device that captured them — losing that device loses those originals even with iCloud Sync on, so back them up separately. NexGenSpec does not host, backup, audit, or guarantee retention of these records, and iCloud Sync is not a backup (deletions sync across the Inspector's devices). The Inspector is fully responsible for backing up records to meet this retention practice (e.g. via iCloud Backup, an external drive, or another system).

        ============================================================
        DATA SAFETY SUMMARY
        ============================================================

        Inspection content — defects, photos, signatures, LiDAR scans, notes, client info, agent info, calendar events, invoices, and PDFs — is stored privately on your iPhone or iPad in the app's sandboxed storage. Other apps cannot read it. NexGenSpec does not receive copies and has no server-side database of inspection data. When iCloud Sync is on (the default), your inspections — the inspection record, report PDFs, thumbnails, and LiDAR floor plans — sync across your own Apple devices through your private iCloud account (Apple CloudKit). Full-resolution photos, videos, and 3D scan files stay on the device that captured them. This data goes only to your iCloud and never to NexGenSpec. Turn on Local-Only mode to keep inspections on a single device. iCloud Sync is not a backup, and deletions sync across your devices.

        The data sent off-device is: (a) your account login (email and an encrypted password, or your Apple ID identifier if using Sign in with Apple) stored in Firebase Authentication, (b) anonymized crash reports sent to Firebase Crashlytics if the app crashes, (c) when an inspection captures weather, the approximate coordinates (~1 km) of your current location, sent to Open-Meteo (open-meteo.com) to fetch current conditions (no account, not linked to your identity, no copy retained by us), and (d) when iCloud Sync is on, your inspections syncing to your own private iCloud account as described above. No advertising data, no inspection content sent to NexGenSpec, no background tracking.
    ...
    public func containsSearchMatch(in id: String) -> Bool? {
        ...
        case "section2":
            ...
            Important: NexGenSpec software stores inspection records on the Inspector's device and, when iCloud Sync is on, syncs the inspection record, report PDFs, thumbnails, and LiDAR floor plans across the Inspector's own Apple devices through the Inspector's private iCloud account. Original full-resolution photos, videos, and 3D scan files never sync and exist only on the device that captured them. iCloud Sync is not a backup. NexGenSpec does not host, backup, audit, or guarantee retention of these records.
            """
        default:
            contentForId = ""
        }
