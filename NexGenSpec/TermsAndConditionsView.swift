...
public struct TermsAndConditionsView: View {
    ...
    public var fullTermsText: String {
        ...
        Important: NexGenSpec software stores inspection records on the Inspector's device and, when iCloud Sync is on, syncs them across the Inspector's own Apple devices through the Inspector's private iCloud account. NexGenSpec does not host, backup, audit, or guarantee retention of these records, and iCloud Sync is not a backup (deletions sync across the Inspector's devices). The Inspector is fully responsible for backing up records to meet this retention practice (e.g. via iCloud Backup, an external drive, or another system).

        ============================================================
        DATA SAFETY SUMMARY
        ============================================================

        Inspection content — defects, photos, signatures, LiDAR scans, notes, client info, agent info, calendar events, invoices, and PDFs — is stored privately on your iPhone or iPad in the app's sandboxed storage. Other apps cannot read it. NexGenSpec does not receive copies and has no server-side database of inspection data. When iCloud Sync is on (the default), your inspections — the inspection record, report PDF, and thumbnails — sync across your own Apple devices through your private iCloud account (Apple CloudKit); this data goes only to your iCloud and never to NexGenSpec. Turn on Local-Only mode to keep inspections on a single device. iCloud Sync is not a backup, and deletions sync across your devices.
